...
def run_cron_scheduler(cron: str, prompt: str, is_repeat: bool) -> str:
    result = schedule_cron(cron, prompt, is_repeat)
...
def run_cron_list() -> str:
    ...
    lines = []
    for job in jobs:
        frequency = "recurring" if job.is_repeat else "one-shot"
